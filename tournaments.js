// VCC Tournaments: signup + check-in + auto bracket trigger
...
(function(){
  ...
  function checkInState(t){
    if(!t.start_date) return { open:false, closed:false, label:'No start time set' };

    const start = new Date(t.start_date).getTime();
    const now = Date.now();
    const checkOpen = start - 30 * 60 * 1000;

    if(now < checkOpen){
      return { open:false, closed:false, label:`Check-in opens ${new Date(checkOpen).toLocaleString()}` };
    }

    if(now >= checkOpen && now <= start){
      return { open:true, closed:false, label:'Check-in open' };
    }

    return { open:false, closed:true, label:'Check-in closed' };
  }
  ...
  async function tryAutoGenerate(tournamentId){
    const result = await supabase.rpc('auto_generate_bracket_if_ready', {
      p_tournament_id:tournamentId
    });
  ...
  async function render(){
    ...
    for(const t of tournaments){
      await tryAutoGenerate(t.id);

      const regs = await loadRegistrations(t.id);
      const state = checkInState(t);
      const check = eligibility(t);
      const myRegistration = currentUser ? regs.find(r => r.profile_id === currentUser.id || myTeams.some(tm => tm.id === r.team_id)) : null;
    ...
    document.querySelectorAll('.signupBtn').forEach(btn => btn.addEventListener('click', () => registerTournament(btn.dataset.id)));
    document.querySelectorAll('.checkInBtn').forEach(btn => btn.addEventListener('click', () => checkIn(btn.dataset.id, btn.dataset.team)));
