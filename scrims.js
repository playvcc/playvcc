// VCC Scrims queue + match room + sound alert fix
...
(function(){
  ...
  async function enterQueue(){
    try{
      ...
      // First look for opponent already waiting.
      ...
      // If no opponent, put/update this team into queue.
    ...
    try{
      ...
      queueBox.innerHTML = data.length ? data.map(q => `
        <div class="identity-row">
          ...
          <strong>${safe(q.region)} / ${safe(q.scrim_type)}</strong>
        </div>
  ...
  async function loadRooms(){
    try{
      const result = await supabase
        .from('match_rooms')
        .select('*, team_a:teams!match_rooms_team_a_id_fkey(*), team_b:teams!match_rooms_team_b_id_fkey(*)')
        .eq('room_type','scrim')
        .order('created_at', { ascending:false })
        .limit(8);

      if(result.error){
        // fallback if FKs are not named
        ...
        roomsBox.innerHTML = (fallback.data || []).length ? fallback.data.map(r => `
      ...
      roomsBox.innerHTML = data.length ? data.map(r => `
        <article class="vcc-card">
          <div class="vcc-panel-title">
            <h2>${safe(r.team_a?.name || 'Team A')} vs ${safe(r.team_b?.name || 'Team B')}</h2>
            <span>${safe(r.status || 'open')}</span>
          </div>
          <p>${safe(r.region || '')} / ${safe(r.scrim_type || '')}</p>
          <a class="btn" href="match-room.html?id=${safe(r.id)}">Open Room</a>
        </article>
      `).join('') : '<div class="log">No rooms yet.</div>';
    }catch(error){
