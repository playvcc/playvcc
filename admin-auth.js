...
function showAdminLock(){
  ...
  document.getElementById('adminCodeBtn').addEventListener('click', () => {
    const value = document.getElementById('adminCodeInput').value.trim();

    if(value === VCC_ADMIN_CODE){
      sessionStorage.setItem('vcc_admin_unlocked', 'yes');
      lock.remove();
    }else{
      document.getElementById('adminCodeStatus').textContent = 'Wrong admin code.';
    }
  });
}
