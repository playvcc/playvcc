-- VCC Group Stage + Invites + Live Match Chat Addon
-- Safe add-on. Does not reset your existing database.
...
for select using (recipient_user_id = auth.uid() or sender_user_id = auth.uid() or recipient_user_id is null);
...
for update using (recipient_user_id = auth.uid() or sender_user_id = auth.uid() or recipient_user_id is null);
