...
-- =========================================
-- VCC SAFE UPDATE SQL
-- This does NOT delete teams, profiles, invites, or scrims.
-- Use this for future updates after your database is already created.
-- =========================================

create extension if not exists "uuid-ossp";

-- Add missing profile fields safely
...
-- Add missing team fields safely
...
-- Add full scrim system fields safely
...
begin
  ...
  if active_count > 0 then
    raise exception 'You are already on an active team.';
  end if;
...
begin
  ...
  if found then
    update scrim_queue set status = 'matched' where id = opponent.id;
...
declare
  ...
  my_team_id uuid;
begin
  select * into match_row from scrim_matches where id = match_id_input;
  ...
  if my_team_id is null then
    raise exception 'Only matched team captains can ready up.';
  end if;
  ...
  where id = match_id_input
    and team_a_ready = true
    and team_b_ready = true;
...
-- Drop/recreate safe policies without deleting data
...
-- =========================================
-- VCC ROSTER MANAGEMENT SAFE UPDATE
-- Does not delete existing data.
-- =========================================

alter table team_memberships add column if not exists left_at timestamptz;
...
-- Optional: make sure restored/removed statuses are allowed by app convention.
